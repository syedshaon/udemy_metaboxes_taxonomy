...
// Function to print Future events with Shortcode API
add_shortcode('upcoming-events', 'upcoming_ecents');

function upcoming_ecents($text){
    if(!is_admin()){
    $args = array(
        'post_type'     => 'events',
        'orderby'       => 'meta_value',
    ...
    while($events->have_posts()){
        $events->the_post();
                ...
                echo get_the_term_list(get_the_ID(), 'type_event','<p><b>Type:</b> ', ', ', '');
...
function metaboxes_events_cmb2(){
    ...
    $metabox_events -> add_field(array(
     ...
     $metabox_events -> add_field(array(
        ...
        'desc'      => __('Event Date(Pict from Calendar)', 'cmb2'),
        'id'        => $prefix.'date',
     ...
     $metabox_events -> add_field(array(
        ...
        'desc'      => __('Enter the number of Setas', 'cmb2'),
        'id'        => $prefix.'seats',
        'type'      => 'text',
    ));
