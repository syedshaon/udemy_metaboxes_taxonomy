			...
			<?php
			 if(is_single()){ ?>
				<div class="extra-information">
					...
					<?php
					$calories = get_post_meta(get_the_ID(), 'input-metabox', true);
					$rating = get_post_meta(get_the_ID(), 'dropdown-metabox', true);
					$description = get_post_meta(get_the_ID(), 'textarea-metabox', true);				 
					?>
						<div class="row">
							<?php
								if($calories){ ?>
							...
							<?php
								...
								<div class="rating small-6 columns">
									<p>Rating: <em><?php echo $rating; ?> Stars</em></p>
								</div>
							<?php }
							?>
						</div>
					<?php
						if($description){ ?>
							<blockquote class="description">Description: <?php echo $description; ?></blockquote>					
					<?php }
					?>
				</div> <!-- Extra infromation ends -->
			<?php } ?>
	<?php }
